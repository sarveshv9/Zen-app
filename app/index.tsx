...
const ROUTINE = [
  { hour: 6, task: "🌞 Wake Up Slowly" }, { hour: 6.5, task: "💧 Drink Warm Water" },
  { hour: 7, task: "🧘 Light Stretching or Yoga" }, { hour: 8, task: "🍵 Herbal Tea & Journaling" },
  { hour: 9, task: "🥣 Healthy Breakfast" }, { hour: 10, task: "📚 Learn Something Calm" },
  { hour: 13, task: "🥗 Light Lunch" }, { hour: 15, task: "🌿 Nature Walk or Break" },
  { hour: 17, task: "📝 Reflect on the Day" }, { hour: 19, task: "🍽 Light Dinner" },
  { hour: 21, task: "🌙 Prepare for Sleep" }, { hour: 22, task: "🛌 Sleep Early" },
];
...
const TASK_THEME_MAP: Record<string, ThemeName> = {
  "🌞 Wake Up Slowly": "pikachu", "💧 Drink Warm Water": "squirtle",
  "🧘 Light Stretching or Yoga": "dragonite", "🍵 Herbal Tea & Journaling": "mew",
  "🥣 Healthy Breakfast": "slowpoke", "📚 Learn Something Calm": "psyduck",
  "🥗 Light Lunch": "charizard", "🌿 Nature Walk or Break": "bulbasaur",
  "📝 Reflect on the Day": "meowth", "🍽 Light Dinner": "jigglypuff",
  "🌙 Prepare for Sleep": "gengar", "🛌 Sleep Early": "snorlax",
};
...
export default function HomeScreen() {
  ...
  // The 'return' statement is now correctly placed within the single component function
  return (
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        <View style={sharedStyles.centeredContainer}>
          ...
          <Animated.View entering={FadeIn.duration(800).delay(400)} style={[styles.imageContainer, animatedImageStyle]}>
            {taskImage && <Animated.Image source={taskImage} style={styles.taskImage} resizeMode="contain" />}
          </Animated.View>
          ...
          <Animated.View entering={FadeIn.duration(600).delay(800)}>
            <Pressable
              style={({ pressed }) => [sharedStyles.primaryButton, styles.startButton, pressed && styles.startButtonPressed]}
              onPress={handleStartDay}>
              <Text style={[sharedStyles.primaryButtonText, styles.startButtonText]}>Start My Day</Text>
            </Pressable>
          </Animated.View>
        </View>
...
const getStyles = (theme: Theme) => StyleSheet.create({
  ...
  time: { fontSize: 64, color: theme.colors.primary, marginBottom: 24, fontFamily: theme.fonts.bold, letterSpacing: -2 },
  taskContainer: { backgroundColor: theme.colors.white, borderRadius: 16, padding: theme.spacing.xl, marginBottom: theme.spacing.xl, alignItems: "center", width: "90%", shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 5 },
  taskLabel: { fontSize: 14, color: theme.colors.secondary, fontFamily: theme.fonts.medium, marginBottom: theme.spacing.xs, textTransform: "uppercase", letterSpacing: 1 },
  task: { fontSize: 20, color: theme.colors.primary, fontFamily: theme.fonts.bold, textAlign: "center" },
  imageContainer: { marginBottom: theme.spacing.xl },
  taskImage: { width: 140, height: 180 },
  quote: { fontSize: 16, fontFamily: theme.fonts.regular, fontStyle: 'italic', color: theme.colors.secondary, marginBottom: theme.spacing.md, textAlign: "center", opacity: 0.8 },
  startButton: { marginTop: theme.spacing.lg, paddingVertical: theme.spacing.lg, paddingHorizontal: 24 },
  startButtonPressed: { transform: [{ scale: 0.98 }], opacity: 0.9 },
